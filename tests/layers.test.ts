...
function separationJob(
  id: string,
  stage: "queued" | "claimed" | "running" | "succeeded" | "failed" | "cancelled",
  sourceVersionId = "original-v1",
) {
  return {
    id,
    capability: { name: "separate", version: "1.0" },
    input_version_ids: [sourceVersionId],
    lifecycle: { current: stage },
  };
}

function bundle(artifacts: unknown[], jobs: unknown[] = []): WorkBundle {
  return {
    work: { id: "work" },
    jobs,
    artifacts,
...
describe("experimental layer source authority", () => {
  ...
  it("exposes exactly vocals, drums, bass, other for one succeeded complete job", () => {
    const work = bundle([
      ...
      stem("bass", "bass-v1"),
      stem("other", "other-v1"),
    ], [separationJob("job-1", "succeeded")]);

    expect(completeLayerJobIds(work, "original-v1")).toEqual(["job-1"]);
    expect(separationJobForSource(work, "original-v1")?.id).toBe("job-1");
  ...
  it("never mixes roles across Jobs or source Versions to manufacture a complete result", () => {
    const work = bundle([
      ...
      separationJob("job-a", "succeeded", "original-v2"),
      separationJob("job-b", "succeeded", "original-v2"),
    ]);
