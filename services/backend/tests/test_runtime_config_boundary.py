...
_MIGRATED_BACKEND_FILES = (
    "services/backend/observability.py",
    "services/backend/worker.py",
    "services/backend/domain/repositories/client.py",
    "services/backend/engines/registry.py",
)
...
def test_env_examples_use_the_canonical_backend_sentry_name():
    for relative_path in (".env.example", "services/backend/.env.example"):
