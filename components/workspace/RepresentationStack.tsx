...
function AnalysisSummary({ onSeek, bpm }: { onSeek: (seconds: number) => void; bpm: number }) {
  const { workspace } = useWorkspace(); const primary = workspace.insights.filter((item) => ["key", "tempo", "time_signature", "audio_tempo"].includes(item.kind)); const chords = workspace.insights.filter((item) => item.kind === "chord").slice(0, 12); const sections = workspace.insights.filter((item) => item.kind === "section").slice(0, 12); const observations = workspace.insights.filter((item) => !["key", "tempo", "time_signature", "audio_tempo", "chord", "section"].includes(item.kind)).slice(0, 8);
  ...
  return <div className="analysis-content"><div className="analysis-facts">{primary.map((item) => <div key={item.id}><span>{item.kind.replaceAll("_", " ")}</span><strong>{item.claim.replace(/^[^:]+:\s*/, "")}</strong><small>{Math.round(item.confidence * 100)}% confidence</small></div>)}</div>{sections.length > 0 && <div className="analysis-block"><h3>Form</h3><div className="rn-chips">{sections.map((item) => <button type="button" className="rn-chip" key={item.id} onClick={() => goTo(item)}>{item.claim}</button>)}</div></div>}{chords.length > 0 && <div className="analysis-block"><h3>Harmonic path</h3><div className="rn-chips">{chords.map((item) => <button type="button" className="rn-chip" key={item.id} onClick={() => goTo(item)}>{item.claim}</button>)}</div></div>}{observations.length > 0 && <div className="analysis-block"><h3>Observations</h3>{observations.map((item) => <button type="button" className="analysis-observation" key={item.id} onClick={() => goTo(item)}><span>{item.claim}</span><small>{Math.round(item.confidence * 100)}% confidence</small></button>)}</div>}</div>;
}
