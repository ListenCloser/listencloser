...
function AnalysisSummary({ onSeek, bpm }: { onSeek: (seconds: number) => void; bpm: number }) {
  const { workspace } = useWorkspace(); const primary = workspace.insights.filter((item) => ["key", "tempo", "time_signature"].includes(item.kind)); const chords = workspace.insights.filter((item) => item.kind === "chord").slice(0, 12); const observations = workspace.insights.filter((item) => !["key", "tempo", "time_signature", "chord"].includes(item.kind)).slice(0, 8);
