...
def handle_analyze(job: Job, client) -> list[str]:
    ...
    if rhythm:
        rid = _create_insight(
            client, input_version.id, "rhythm",
            f"{rhythm.get('rhythmic_density', 0)} notes/sec · {round(float(rhythm.get('syncopation_ratio', 0)) * 100)}% off-beat on the inferred grid",
            evidence=rhythm, confidence=0.65, job=job, owner_id=owner_id,
        )
        insight_ids.append(str(rid))
    ...
    if melody:
        mid = _create_insight(
            client, input_version.id, "melody",
            f"Range: MIDI {melody.get('low_pitch')}–{melody.get('high_pitch')} · {round(float(melody.get('stepwise_ratio', 0)) * 100)}% stepwise motion",
            evidence=melody, confidence=0.6, job=job, owner_id=owner_id,
        )
    ...
    if voice_leading:
        vid = _create_insight(
            client, input_version.id, "voice_leading", voice_leading.get("motion_summary", "Voice-leading summary"),
            evidence=voice_leading, confidence=0.55, job=job, owner_id=owner_id,
        )
    ...
    for modulation in (analysis.get("modulations") or [])[:12]:
        ...
        mid = _create_insight(
            client, input_version.id, "modulation", f"{modulation.get('from_key', '?')} → {modulation.get('to_key', '?')}",
            evidence=modulation, span=Span(start_seconds=position), confidence=0.5, job=job, owner_id=owner_id,
        )
        insight_ids.append(str(mid))
