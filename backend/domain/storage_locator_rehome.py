...
def _destination_key(
    ...
    return (
        f"{owner_id}/{project_id}/{artifact_id}/" f"{replacement_id.hex}{_safe_suffix(source_key)}"
    )
...
def _insert(client: Client, source_version_id: UUID, version: Version) -> Version:
    try:
        result = client.rpc(
            "publish_storage_rehome_version",
            {
                "p_source_version_id": str(source_version_id),
                "p_version": version.model_dump(mode="json"),
            },
        ).execute()
    except Exception:
...
def rehome_selected_storage(
    ...
    *,
    ...
    for detail in selected:
        ...
        created = _insert(client, source.id, replacement)
        _verify_trusted(
            created,
            owner_id=owner_id,
            project_id=project_id,
