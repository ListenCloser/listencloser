...
class PgmqJobWorker(FencedJobWorker):
    ...
    def _receive_next_job(self) -> dict[str, Any] | None:
        """Receive one PGMQ message and install its fresh execution identity."""
    ...
    def _finish_delivery(self, job_id: str, token: str, msg_id: int) -> None:
        ...
        try:
            ...
            # Fail safe: never acknowledge on uncertainty. Visibility expiry
            # redelivers and durable Job state decides the next disposition.
            logger.exception(
    ...
    def _execute_job(self, job_row: dict) -> None:
        ...
        if not token_value or msg_value is None:
            raise RuntimeError("PGMQ execution requires a received delivery identity")
        ...
        try:
            ...
            # The RPC is token-guarded. A stale attempt cannot acknowledge or
            # alter visibility for a successor execution.
