"""On-demand query boundary for same-work perceptual relations.

The persisted source of truth remains the immutable ``perceptual_series`` analysis
report. User-selected comparisons and similar-moment searches are cheap,
deterministic derivations over that report, so this module resolves and validates
the latest matching report without persisting duplicate relation state.
"""
...
def find_persisted_similar_moments(
    ...
    *,
    ...
    """Resolve and validate persisted evidence, then propose same-Work candidates."""

    source_artifact, source_reasons = _source_artifact(snapshot, source_version)
    if source_artifact is None:
        return _similar_result("failed", reasons=source_reasons)
