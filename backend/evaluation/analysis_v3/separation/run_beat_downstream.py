"""Run the separation -> beat downstream gate on a fixed BabySlakh subset.
...
def _reference_beats(midi_path: str) -> list[float]:
    beats = [float(value) for value in pretty_midi.PrettyMIDI(midi_path).get_beats()]
    if not beats:
        raise ValueError(f"No synthesis beat grid in {midi_path}")
    return beats
...
def run_babyslakh_beat_gate(
    *,
    ...
    for track_id in track_ids:
        ...
        mixture, sample_rate = _load_audio(resolved.audio_path, target_sr=44100)
        ...
        reference_beats = _reference_beats(resolved.reference_midi_path)
...
def main() -> None:
    ...
    payload = run_babyslakh_beat_gate(device=args.device)
