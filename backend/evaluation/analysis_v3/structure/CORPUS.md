...
Use a small fixed **SongFormBench-CN / `BC`** subset before expanding the corpus. `BHX` is derived
from HarmonixSet and is training-overlapping for Harmonix-trained All-In-One checkpoints, so it must
not be the first independent-quality claim.

The first corpus should contain 8 BC songs selected without looking at candidate outputs.
...
- CN mel spectrograms under the dataset's CN mel assets;
...
## 2. Freeze a deterministic BC source-ID subset

Before reconstructing audio, select the corpus from the canonical index:
...
Selection policy `lexicographic_source_id_v1` sorts canonical BC source IDs and takes the first N.
...
This is deliberately boring: it prevents model-output cherry-picking and makes an upstream index
change visible even when the requested subset name remains `BC`.

Commit the small selection/provenance JSON if redistribution of that metadata remains covered by the
upstream CC BY 4.0 dataset license. Do **not** commit reconstructed or original song audio by default.
...
3. other documented local audio -> `audio_provenance=local_unknown` only when its exact provenance
   cannot be established.

For mel reconstruction, use the upstream `utils/CN/infer.py` implementation with
...
Every compared candidate must receive the **same audio files**. Never compare All-In-One on original
audio with SongFormer on a reconstruction and call the difference a model result.

## 4. Build the hello-ai evaluation manifest

Make reconstructed/local audio resolve to each selected entry's canonical `audio_path`, then use the
existing builder against the filtered index:
...
fixed BC8 run. Missing rows stay explicit; do not replace them with hand-picked alternatives after
seeing model outputs. If a source is unusable, version a new corpus selection and explain why.
