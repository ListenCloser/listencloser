...
the session architecture using only the installed 1.1.0 public/internal
objects, without changing the production dependency or product behavior.
"""
...
def benchmark(input_path: Path, trials: int = 3) -> dict[str, Any]:
    ...
    cold_one_shot_s, baseline = _timed(lambda: chord_recognition(str(input_path)))
    baseline_sha = _output_sha256(baseline)
    ...
    for _ in range(trials):
        ...
        retained_equivalent.append(_output_sha256(output) == baseline_sha and output == baseline)
    ...
    for _ in range(trials):
        ...
        warm_one_shot_equivalent.append(_output_sha256(output) == baseline_sha and output == baseline)
    ...
    return {
        ...
        "output_signature": {
            "segment_count": len(baseline),
            "sha256": baseline_sha,
        },
        "outputs_equivalent": all(retained_equivalent) and all(warm_one_shot_equivalent),
        "cold_one_shot_seconds": round(cold_one_shot_s, 6),
