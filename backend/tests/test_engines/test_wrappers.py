...
class TestTranscriptionWrapper:
    def test_wrapper_resolves_engine_via_registry(self):
        ...
        fake_result = TranscriptionResult(
            midi=b"midi", wav=b"wav", notes=fake_notes, num_notes=1,
            cleanup_report={"kept": 1}, provenance=engine.provenance,
        )
    ...
    def test_wrapper_passes_parameters(self):
        ...
        engine = BasicPitchEngine()
        engine.transcribe = MagicMock(return_value=TranscriptionResult(
            midi=b"x", wav=b"x", notes=[], num_notes=0,
            cleanup_report={}, provenance=engine.provenance,
        ))
...
class TestBeatWrapper:
    def test_wrapper_resolves_engine_via_registry(self):
        ...
        engine.analyze = MagicMock(return_value=BeatTrackingResult(
            bpm=120.0, beats=[0.0, 0.5], downbeats=None,
            beat_positions=[0, 1], provenance=engine.provenance,
        ))
...
class TestNotationWrapper:
    def test_wrapper_resolves_engine_via_registry(self):
        ...
        engine.convert = MagicMock(return_value=NotationResult(
            notation_midi=b"midi", musicxml=b"xml",
            quantization_report={"notes": 5}, provenance=engine.provenance,
        ))
