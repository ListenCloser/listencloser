...
- Branch refresh, auto-merge enablement, and enqueueing are owned by the PAT-based reconciler (`scripts/reconcile-pr-queue.sh`, run on a schedule with `QUEUE_BOT_TOKEN` or directly by an agent). Agents must not manually merge/rebase/refresh branches or babysit the queue; the reconciler and the native merge queue own that. Agents only re-engage on `needs-reconcile`, a failed `merge_group`, or a failed required check that needs reasoning.
- Mechanical, deterministic CI failures (Ruff safe fixes/format, ESLint `--fix`, generated-contract regeneration) are auto-repaired by the in-workflow autofix step using the same PAT; an agent should not spend time re-pushing formatting or generated-file fixes. Autofix never applies `--unsafe-fixes` or behavioral changes. A format-only or safely fixable lint failure is not a blocker: verify the autofix pushed a corrected head, otherwise it is a semantic failure.
- The expensive whole-system proofs (real-stack E2E, database integration, backend image, full E2E, production build) run on the `merge_group` SHA, not on every PR head. PR heads run the fast admission tier; agents must not recreate the full gate by polling independent workflows.
