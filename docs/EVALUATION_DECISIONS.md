...
| Notation / Score fidelity | **OSS CANDIDATE INTEGRATION — product gate next** | #509 found a real notation-stage timing bug, but the current product still has a broader downstream performance-MIDI→readable-score problem even when Piano Roll evidence is useful. #700 re-established the boundary: canonical performance MIDI stays faithful; Score is a derived interpretation. #707 integrates MuseScore Studio 4 as the first opt-in production-shaped OSS baseline without claiming it consumes the Beat This grid. PM2S remains the next component challenger; MIDI2ScoreTransformer remains attractive but blocked on clear code/checkpoint licensing and heavier dependency requirements. | Keep `music21` as the notation default until the MuseScore candidate passes native amd64/arm64 execution and real product inspection. **Freeze further bespoke quantization/staffing tuning meanwhile.** If MuseScore wins, promote it behind rollback and delete superseded custom score-interpretation stages rather than maintaining two heuristic systems. | Run the canonical real-piano Work through current vs MuseScore in the actual Score/OSMD path. Inspect rhythmic readability, ties/durations, staff assignment, note loss, measure structure and playback sanity, then end with **SHIP / KEEP / REJECT**. Do not add another notation-evaluation abstraction before this product comparison. |
...
1. **Finish #707's runnable MuseScore gate, then compare current-vs-MuseScore in the actual product Score path.** Promote/delete bespoke stages if it wins; reject quickly and move to the next challenger if it does not.
