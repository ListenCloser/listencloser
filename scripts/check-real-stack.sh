...
docker run -d --rm --name "$API_CONTAINER" --network "$SUPABASE_NETWORK" \
  ...
  -e "SUPABASE_URL=$CONTAINER_SUPABASE_URL" -e "SUPABASE_PUBLIC_URL=$SUPABASE_URL" \
  -e "SUPABASE_ANON_KEY=$SUPABASE_ANON_KEY" -e "SUPABASE_SERVICE_ROLE_KEY=$SUPABASE_SERVICE_ROLE_KEY" \
  -e "HARMONY_ENGINE=$HARMONY_ENGINE" \
