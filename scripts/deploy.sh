...
preseed_librosa_numba_cache() {
  ...
  if ! docker compose -f "$COMPOSE" run --rm --no-deps --cap-add CHOWN --cap-add DAC_OVERRIDE --user root --entrypoint sh worker \
...
docker compose -f "$COMPOSE" run --rm --no-deps --cap-add CHOWN --cap-add DAC_OVERRIDE --user root --entrypoint sh worker \
